...
export default function ClientPushNotifier() {
  ...
  useEffect(() => {
    ...
    const checkNotifications = () => {
      fetch(`http://localhost/figtop-api/get_unread_notifications.php?client_id=${activeSessionToken}&role=client`)
        .then((res) => res.json())
        .then((alerts) => {
          if (alerts && alerts.length > 0) {
            ...
            if (latest && latest.id && !shownAlerts.includes(latest.id)) {
              ...
              fetch("http://localhost/figtop-api/clear_notifications.php", {
                method: "POST",
